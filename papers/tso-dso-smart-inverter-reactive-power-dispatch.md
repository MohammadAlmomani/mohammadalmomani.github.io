...
The proposed method models IEEE 1547-compliant smart inverter control modes, including:

* Volt-VAR control
* Volt-Watt control
* Watt-VAR control

The framework uses a sensitivity-aware mixed-integer linear programming (MILP) formulation, Special Ordered Sets of type 1 (SOS1), Recursive Least Squares (RLS) estimation, and an observable/unobservable network reformulation to support practical real-time implementation under limited measurement conditions.
...
1. **Unified MILP formulation for IEEE 1547 smart inverter control modes**
   The paper formulates Volt-VAR, Volt-Watt, and Watt-VAR droop control modes using piecewise-linear constraints and Big-M modeling.

2. **Real-time hierarchical optimization using SOS1**
   The method replaces large binary-variable structures with SOS1-based mode and segment selection, improving computational scalability.
...
The proposed method follows a three-level coordination structure:

### 1. DSO Aggregation

The DSO estimates the distribution system reactive power capability range at the TSO-DSO interface.
...
The DSO allocates the requested reactive power among DER smart inverters while respecting local inverter operating modes, droop curves, voltage constraints, and active-power limits.

---

## Smart Inverter Control Modes

The paper models three IEEE 1547 droop-based inverter modes:

### Volt-VAR Control

Reactive power is adjusted based on local voltage magnitude.

**Search terms:** Volt-VAR optimization, Q(V) control, IEEE 1547 Volt-VAR, smart inverter reactive power support.

### Volt-Watt Control

Active power is curtailed when voltage exceeds specified limits.

**Search terms:** Volt-Watt control, P(V) smart inverter, PV curtailment, overvoltage mitigation.

### Watt-VAR Control

Reactive power is adjusted based on active power output.

**Search terms:** Watt-VAR control, Q(P) control, smart inverter power-dependent reactive support.
...
* The proposed optimized droop-based operation reaches the maximum available active power while preserving useful reactive flexibility.
...
* The coordinated multi-mode smart-inverter dispatch improves transmission-voltage profiles under contingency conditions.
...
TSO-DSO coordinated reactive power dispatch is a control framework where the distribution system estimates its available reactive power flexibility from DERs and communicates this capability to the transmission operator. The transmission operator then requests reactive power support, and the distribution operator disaggregates the request among local DERs.
...
SOS1 is used to improve the computational scalability of the MILP formulation. It helps select one active control mode and one active droop segment while reducing solver complexity compared with a full binary-variable formulation.

### How does the method work with limited measurements?

The distribution network is partitioned into observable and unobservable parts. Recursive Least Squares is then used to estimate unknown parameters that capture the effect of unobservable states on observable buses.
...
* IEEE 1547 inverter modeling
* Real-time voltage regulation
* PV hosting capacity
* Inverter-based resource integration
* Active distribution networks
* Limited-observability distribution system optimization
...
* **Code repository:** Add GitHub repository link here when available.
* **Dataset / test cases:** Add Zenodo, GitHub, or OpenDSS test-case link here when available.
...
A good repository name for code related to this paper is:

```text
tso-dso-smart-inverter-dispatch
```

Alternative names:

```text
smart-inverter-reactive-power-dispatch
ieee1547-smart-inverter-milp-dispatch
tso-dso-reactive-dispatch-opendss
```
...
If you use the TSO-DSO coordination framework, smart-inverter MILP formulation, IEEE 1547 droop-control modeling, SOS1 implementation, RLS limited-observability method, or OpenDSS-based test cases, please cite this paper.
